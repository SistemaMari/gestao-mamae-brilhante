...
BEGIN
  ...
  cenarios_por_paciente AS (
    SELECT
      c.paciente_id,
      bool_or(c.cenario_clinico IN ('cenario_1','cenario_6','cenario_6b'))      AS tem_dmg,
      bool_or(c.cenario_clinico IN ('cenario_8','8'))                            AS tem_overt,
      bool_or(c.cenario_clinico = 'cenario_1')                                   AS dx_retorno1,
      bool_or(c.cenario_clinico = 'cenario_6')                                   AS dx_gtt_janela,
      bool_or(c.cenario_clinico = 'cenario_6b')                                  AS dx_gtt_tardio,
      bool_or(c.cenario_clinico = 'cenario_3')                                   AS iniciou_insulina,
      bool_or(c.cenario_clinico = 'cenario_4')                                   AS insulina_ok,
      bool_or(c.cenario_clinico = 'cenario_7')                                   AS cenario7,
      -- IG média no momento do diagnóstico, por momento.
      avg(CASE WHEN c.cenario_clinico='cenario_1'
               THEN coalesce(c.ig_semanas,0) + coalesce(c.ig_dias,0)/7.0 END)    AS ig_dx_retorno1,
      avg(CASE WHEN c.cenario_clinico='cenario_6'
               THEN coalesce(c.ig_semanas,0) + coalesce(c.ig_dias,0)/7.0 END)    AS ig_dx_gtt_janela,
      avg(CASE WHEN c.cenario_clinico='cenario_6b'
  ...
  evol_dx AS (
    ...
    FROM public.consultas c
    WHERE c.cenario_clinico IN ('cenario_1','cenario_6','cenario_6b','cenario_8','8')
